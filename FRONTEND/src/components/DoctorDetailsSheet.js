      ...
      <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="p-6 border-b border-gray-100 flex items-start justify-between bg-gray-50/50">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-xl font-bold text-gray-900">Dr. {doctor.firstName} {doctor.lastName}</h2>
                    <p className="text-sm text-gray-500">{doctor.specialty || 'General Physician'}</p>
                    <div className="flex gap-2 mt-2">
                        <span className={`px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-700`}>
                            {doctor.department || 'General'}
                        </span>
                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${doctor.status === 'Available' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-700'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg border border-gray-100">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-medium text-gray-900">{doctor.mobileNumber || 'N/A'}</p>
                        </div>
                    </div>
            ...
            {/* Professional Info */}
            <div>
                 <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Professional Details</h3>
                 <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="p-4 bg-purple-50 rounded-xl border border-purple-100">
                        ...
                        <p className="text-sm font-medium text-gray-900">{doctor.department || 'OPD'}</p>
                    </div>
                 </div>
            </div>
            ...
            <div>
                <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Performance Overview</h3>
                <div className="p-4 border border-gray-100 rounded-xl bg-white shadow-sm">
                    ...
                    <p className="text-xs text-gray-400 text-right">Based on 120 reviews</p>
                </div>
            </div>

            {/* Activity Mock */}
             <div>
                <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Today's Schedule</h3>
                <div className="space-y-3">
                     {['09:00 AM - Consultation', '11:30 AM - Surgery', '02:00 PM - Rounds'].map((slot, i) => (
                        <div key={i} className="flex items-center gap-3 p-3 border border-gray-100 rounded-lg">
                             <div className="text-blue-500"><FaCalendarCheck /></div>
